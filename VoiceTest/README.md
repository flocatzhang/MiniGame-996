...
**黄色档是 11025 Hz**（二次交付后蓝色也是）**。** 白与橙是 48000。11 kHz 采样率意味着高频只到 5.5 kHz，听感会明显比另外三条闷。它偏偏是四档里的第二好档位，本该比白蓝更亮更有质感。这一条改不了文件就只能忍，**如果后面还有一次重新生成的机会，优先换它**。
...
填这组起始值（**这是首次交付时的值，白蓝两档在二次交付后已经重算，当前生效值见本节末尾**）：
...
### 二次交付后白蓝两档已重算：当前是 0.36 / 0.52 / 1.00 / 0.90

上面的 0.50 / 0.46 之所以是"蓝比白低"，唯一原因是当时的蓝色素材比白色响 4 dB，要压回去。二次交付把**那条素材挪到了白色档**，蓝色换成一条新的 11025 Hz 素材，于是压它的 0.46 留在了蓝色上，补偿加到了错误的一档：按 400 毫秒短时 RMS 实测，四档生效响度成了 −14.5 / −15.4 / −11.3 / −8.2 dB，**白到蓝往下走 1 dB**，第一档升级听不出变化。

现在配置里是白 **0.36** / 蓝 **0.52**（黄橙不动），四档回到 −17.4 / −14.4 / −11.3 / −8.2 dB，约 3 dB 一级。黄色仍然只能留 1.0，它是四档里最轻的一条而真峰值已顶到 0 dBFS 以上，整个阶梯只能靠压低其余三档来搭。

**这组数字只是把测量对齐，验收动作不变**：把四个音连着播一遍，听得出由轻到重才算过。换掉任何一档素材，这两个补偿值都要重算一次——它们跟的是素材响度，不是档位。
...
- [ ] 四档掉落按第四节填 `volume`（0.36 / 0.52 / 1.00 / 0.90），掉落总线留 3 dB 余量
